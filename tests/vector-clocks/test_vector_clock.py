...
def test_update(vector_clock):
    vector_clock.update({"1": 2, "2": 3})
    assert vector_clock.clock["1"] == 2, "Update failed for node1"
    assert vector_clock.clock["2"] == 3, "Update failed for node2"
    assert vector_clock.clock[vector_clock.node_id] == 1, "Update failed for local node"
