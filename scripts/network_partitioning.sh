...
curl -s -X POST "$API_NODE1/set/$TEST_KEY" \
    -H "Content-Type: application/json" \
    -H "$HEADER_API_KEY" \
    -d "{\"value\": \"$VALUE_NODE1\"}"

echo "Updating key on partition2 (node3)..."
curl -s -X POST "$API_NODE3/set/$TEST_KEY" \
    -H "Content-Type: application/json" \
    -H "$HEADER_API_KEY" \
    -d "{\"value\": \"$VALUE_NODE3\"}"
...
curl -s "$API_NODE1/get/$TEST_KEY" -H "$HEADER_API_KEY"
echo ""
...
echo ""
echo "Node2:"
curl -s "$API_NODE2/get/$TEST_KEY" -H "$HEADER_API_KEY"
echo ""
echo "Node3:"
